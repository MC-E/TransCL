...
#     assert bool(model_name) ^ bool(weights_path), 'Expected exactly one of model_name or weights_path'
    ...
    if weights_path is None:
        ...
        print('Have weight: ', weights_path)
    ...
    if resize_positional_embedding: 
        posemb = state_dict['state_dict']['module.positional_embedding.pos_embedding'].cpu()   #state_dict['positional_embedding.pos_embedding'] # edit for new
        posemb_new = model.state_dict()['positional_embedding.pos_embedding']
#         print(posemb,posemb_new)
        state_dict['state_dict']['module.positional_embedding.pos_embedding'] = \
            resize_positional_embedding_(posemb=posemb, posemb_new=posemb_new, 
                has_class_token=hasattr(model, 'class_token'))# edit for new
...
#                 has_class_token=hasattr(model, 'class_token'))
    ...
    # Load state dict
#     state_dict = state_dict['state_dict']   # edit for new
    if 'state_dict' in state_dict:
        ret = model.load_state_dict({k.replace('module.',''):state_dict['state_dict'][k] for k in state_dict['state_dict']}, strict=False) # edit for new
    else:   
